...
// M9 (F10): 1 -> 0. This sleepy remote NEVER self-starts OTA on network-up;
// the app (ota_trigger.c) starts a session on wake at most once per
// OTA_QUERY_MIN_INTERVAL_S, or on the PLUS+MINUS 10 s manual trigger.
...
#define EMBER_AF_PLUGIN_OTA_CLIENT_QUERY_DELAY_MINUTES   1440
...
// M9 (F10): 10 -> 1440. Backstop like QUERY_DELAY above: when no OTA server
// answers discovery the plugin re-arms a rediscovery timer; keep that at one
// day so a coordinator without an OTA server can't cause 10-minute wakeups.
// (ota_trigger.c ends an idle session after OTA_QUERY_GRACE_S anyway.)
#define EMBER_AF_PLUGIN_OTA_CLIENT_SERVER_DISCOVERY_DELAY_MINUTES   1440
